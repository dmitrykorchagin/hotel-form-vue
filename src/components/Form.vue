            ...
            <v-card-text>
              ...
              <v-form ref="form">
                ...
                <v-text-field
                  ...
                  type="number"
                />
                <v-checkbox label="Дети 5-12 лет" v-model="children" hide-details />
                <v-text-field
                  v-if="children"
                  v-model="childrenVal"
                ...
                <v-select :items="tariffs" label="Тип номера" v-model="tariff" :rules="[required]" />
                <div class="title mt-4">Выберите даты размещения</div>
                <div class="d-flex">
                  <DatePicker label="Дата заезда" v-model="date" min="2020-05-15" max="2020-09-15"></DatePicker>
                  <DatePicker label="Дата выезда" v-model="date2" :min="minDate" max="2020-09-15"></DatePicker>
                </div>
                <div v-if="tariff">
                  <v-text-field
                    v-model="fullName"
...
export default {
  ...
  data() {
    return {
      ...
      priceDay: null,
      minDate: '2020-05-16',
      max3: v => (Number((Number(v) / (Number(this.babyVal) + Number(this.childrenVal))).toFixed(2)) >= 0.33) || 'На одного взрослого не более 3 детей от 0-12 лет',
  ...
  watch: {
    ...
    date() {
      ...
      if (+new Date(this.date) >= +new Date(this.date2)) this.date2 = this.minDate
    }
  },
  methods: {
    ...
    priceOneDay() {
      const adultPrice = this.getPriceAdult(this.adultsVal, this.getSeason(this.date), this.tariff)
      const childPrice = this.getPriceChild(this.childrenVal, this.getSeason(this.date), this.tariff)
    ...
    price() {
      ...
      return `Стоимость отдыха с ${d1} по ${d2} ${this.priceOneDay() * days} руб.`
    },
    pay() {
